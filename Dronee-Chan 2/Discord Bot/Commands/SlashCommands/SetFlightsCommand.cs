...
namespace Dronee_Chan_2.Discord_Bot.Commands.SlashCommands
{
    internal class SetFlightsCommand : ApplicationCommandModule
    {
        ...
        public async Task SetFlights(CommandContext ctx, [Option("ID", "The Discord UUID of the person to set the flights of")] string userIDS,
                                                           [Option("Amount", "The amount to set the flights to")] long amountL)
        {
            ...
            user.FlightsAmount = amount;

            EventManager.SaveUser(user);

            await ctx.EditResponseAsync(new DiscordWebhookBuilder().WithContent("Flights of " + ctx.Guild.Members[user.DiscordUUID].Username + " has been updated to " + amount));
        }
    }
}
